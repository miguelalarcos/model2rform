...
(function() {
  ...
  _make_autorun = function(form_name, klass, parent) {
    return function() {
      var id, initial, initials, obj, path, path_, _ref;
      if (parent !== null) {
        id = Session.get(parent + '_object_id');
        path_ = Session.get(form_name + '_object_id').split('.');
        path = path_.slice(0, -1);
        initial = path_.slice(-1)[0];
        console.log('initial', initial);
      } else {
        _ref = Session.get(form_name + '_object_id').split('.'), id = _ref[0], initial = _ref[1];
        path = [];
      }
